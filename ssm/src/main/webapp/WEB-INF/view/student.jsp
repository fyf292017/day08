...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								...
								<td>学生生日：</td>
								<td>
									<input placeholder="学生生日"id="txt_search_sBirth" name="sBirth" class="laydate-icon form-control layer-date"/>
								</td>
							</tr>
							<tr>
								<td>角色：</td>
	...
	function initEditUser(getSelection){
		...
		$("input[value='"+sSex+"']").prop("checked",true);
		
		var sHobby=getSelection.sHobby;
		var hArr=sHobby.split(",");
		for (var i = 0; i < hArr.length; i++) {
			$("input[value='"+hArr[i]+"']").prop("checked",true);
		}
		$('#txt_search_sBirth').val(new Date(getSelection.sBirth).Format('yyyy-MM-dd'));
